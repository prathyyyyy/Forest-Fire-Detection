...
def augmented_batch_generator(images_paths, labels, batch_size, preprocessing, augment, image_size=(224, 224)):
    ...
    while 1:
        ...
        for i in range(0, number_samples, batch_size):
            ...
            batch = list(map(
                lambda x: tf.keras.utils.load_img(x, target_size=image_size),
            ...
            if augment:
                ...
                batch = np.array(list(map(
                    lambda x: data_transformer.random_transform(keras.preprocessing.image.img_to_array(x)),
                    batch
                )))
            else:
                # converting the loaded images to numpy arrays
                batch = np.array(list(map(
                    lambda x: tf.keras.utils.img_to_array(x),
                    batch
                )))
...
train_simpler_inception_based_model(dataset_path="setup/datasets/all",nbr_epochs=30,percentage=0.9,batch_size=24,learning_rate=0.001)